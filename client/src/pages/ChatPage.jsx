...
export function ChatPage() {
  ...
  async function handleResetProfile() {
    if (
      !confirm(
        '¿Seguro que quieres borrar tus datos de este año y empezar de nuevo? Esto también eliminará tus reportes generados.'
      )
    )
      return;
    try {
      const taxYears = await apiFetch('/api/tax-years');
      if (!taxYears.length) throw new Error('No hay ningún año gravable configurado todavía.');
      await apiFetch(`/api/tax-profiles/${taxYears[0].year}`, { method: 'DELETE' });
      const conv = await apiFetch('/api/conversations', { method: 'POST' });
      showToast('Tus datos fueron reiniciados.', 'success');
    ...
    <div className="chat-page">
      ...
      {profileComplete && (
        <div className="chat-complete-bar">
          ...
          <button className="ghost-button" onClick={handleResetProfile}>
            Empezar de nuevo
          </button>
        </div>
      )}
